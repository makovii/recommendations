...
class UnwindCreateOperation extends operations_1.MutationOperation {
    ...
    transpile(context) {
        ...
        const unwindCreateClauses = cypher_builder_1.default.concat(createClause, mergeClause, ...nestedSubqueries, ...authorizationClauses, ...(cardinalityClauses.length ? [new cypher_builder_1.default.With(nestedContext.target), ...cardinalityClauses] : []));
        let subQueryClause;
        if (this.isNested) {
            subQueryClause = cypher_builder_1.default.concat(unwindCreateClauses, new cypher_builder_1.default.Return([cypher_builder_1.default.collect(cypher_builder_1.default.Null), new cypher_builder_1.default.Variable()]));
        }
        else {
            subQueryClause = new cypher_builder_1.default.Call(cypher_builder_1.default.concat(unwindCreateClauses, new cypher_builder_1.default.Return(nestedContext.target))).importWith(this.unwindVariable);
        }
        ...
        const clauses = cypher_builder_1.default.concat(unwindClause, subQueryClause, ...this.getProjectionClause(projectionContext));
